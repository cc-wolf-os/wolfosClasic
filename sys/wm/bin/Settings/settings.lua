...
local fileUtils = require(".sys.lib.utils.file")
...
local generalFunctions = require(".sys.wm.bin.Settings.generalSettingsFunctions")

local cachedPages = {
  ["index.json"] = fileUtils.readJSON("/sys/wm/bin/Settings/Pages/index.json"),
}
...
local function navigate(page)
  ...
  if cachedPages[page] == nil then
    if page:gmatch(".json$") then
      cachedPages[page] = fileUtils.readJSON("/sys/wm/bin/Settings/Pages/" .. page)
    end
  end
