-- Module imports
local logger = require(".sys.lib.log")
local utils = require(".sys.lib.utils")
local RegistryReader = require(".sys.lib.Registry.Reader")
...
local windowRenderer = require(".sys.wm.bin.WindowManagerModules.WindowRenderer"):new(logger, buffer, displayOrder, processes)
local windowEvents = require(".sys.wm.bin.WindowManagerModules.WindowEvents"):new(logger, buffer, wm)
local menu = require(".sys.wm.bin.WindowManagerModules.Menu"):new(logger, buffer, wm)
...
xpcall(function()
    ...
    newProcess.coroutine = coroutine.create(function()
      ...
      xpcall(function()
        if fs.exists(path) then
          ...
          wm.addProcess("/sys/wm/bin/Prompts/Error.lua", {
            isCentered = true,
            w = 32,
            h = 9,
            hideMinimize = true,
        ...
        if endedGracefully == false then
          ...
          wm.addProcess("/sys/wm/bin/processStopped.lua", {
            env = {
              wmProcessStopInfo = {
  ...
  function wm.reloadModules()
    windowRenderer = require(".sys.wm.bin.WindowManagerModules.WindowRenderer"):new(logger, buffer, displayOrder, processes)
    windowEvents = require(".sys.wm.bin.WindowManagerModules.WindowEvents"):new(logger, buffer, wm)
    menu = require(".sys.wm.bin.WindowManagerModules.Menu"):new(logger, buffer, wm)    

    menu:init()

    wm.addProcess("/sys/wm/bin/Prompts/Info.lua", {
      isCentered = true,
      w = 32,
      h = 9,
      hideMinimize = true,
  ...
  -- Begin services
  wm.addProcess("/sys/wm/bin/Services/ServiceWorker.lua", {isService = true})
